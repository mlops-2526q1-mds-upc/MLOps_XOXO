...
TRAIN_FILE = Path(params['dataset']['processed_dir']) / "splits/train.csv"
VAL_FILE = Path(params['dataset']['processed_dir']) / "splits/val.csv"
OUTPUT_DIR = Path("models/age_gender")
...
class SimpleModel(nn.Module):
    ...
    def __init__(self, num_outputs):
        ...
        self.backbone.classifier = nn.Sequential(
            nn.Dropout(p=params['model']['dropout']),
    ...
    with mlflow.start_run(run_name=f"{task}_training") as run:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            print(f"Epoch {epoch+1}/{NUM_EPOCHS} - "
                  f"TrainLoss: {train_loss:.4f} | ValLoss: {val_loss:.4f} | Metric: {metric:.4f}")
